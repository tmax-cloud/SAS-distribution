...
- SAS admin ddl 변경 - 최적화
- 대용량 WebSocketFrame 처리 지원 (BinaryWSFrame)
- SAS Trial 추가 (Docker, Docker-Compose 기반)
- App 재기동 시 DBCP 자동 재생성 지원, App 삭제 시 연관된 dynamic datasource 정리
- Controller refactoring - ServiceManager 의 callAsync, callSync 메서드 사용하여 호출
- Custom Gateway - Http Outbound / Inbound message 처리 기능 추가, JSON message mapping 지원
- SMS - message를 publisher가 제공하는 JSON format으로 변경
