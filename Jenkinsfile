...
Super-App-Server-0.0.9 버전에서는 Internal-Controller 에 대한 업로드와
해당 컨트롤러의 서비스 콜이 가능합니다. 
현재는 마스터를 통한 서비스 콜만 가능하며, 타 서비스에서 컨트롤러의 서비스를 부르는
nested 구조의 콜은 구현되지 않은 상태입니다.

SASAdmin client 사용 간 버그와 Controller 서비스 콜에 관련된 이슈를
지속적으로 수정하여 다음 버전 배포때 반영하겠습니다.
